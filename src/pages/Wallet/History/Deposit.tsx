...
const HistoryDeposit = () => {
  ...
  const loadDepositUser = () => {
    setState(state => ({ ...state, loading: true, deposits: [] }));
    WalletApi.getDepositByUser(state.page, state.pageSize)
      .then(response => {
        if (response.data)
          setState(state => ({
            ...state,
            deposits: response.data.items,
          ...
          <Table bordered hover variant="dark">
            ...
            <tbody>
              {state.deposits.length > 0 ? (
                state.deposits.map((item: any, key) => (
                  <tr key={key}>
...
export default React.memo(HistoryDeposit);
